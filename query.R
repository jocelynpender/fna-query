...
ask_query_url <- function(query_string) {
  # Concatenate a query_string with the http api URL for a Semantic MediaWiki
  # instance
...
query_property_texts <- function(query_results, property) {
  ...
  grepped_property_text_labels = property_text_labels %>% map(~names(unlisted_printouts)[grepl(., 
    names(unlisted_printouts))]) %>% unlist
  if (length(grepped_property_text_labels) > length(property_text)) {
    # If the above returns no hits, the property text may be stored as 'fulltext'
    ...
    if (any(grepl("\\d$", grepped_property_text_labels))) {
...
ask_query_titles_properties <- function(query_string, output_file_name) {
  ...
  # Examples: query_string =
  # '[[Authority::Miller]]|?Taxon family|?Volume' query_string =
  # '[[Distribution::Ont.]][[Author::Geoffrey A. Levin]]|?Taxon
  # family|?Volume|?Illustration|?Distribution' query_string =
  # '[[Distribution::Ont.]][[Author::Geoffrey A. Levin]]|?Taxon
  # family|?Volume|?Distribution' 
