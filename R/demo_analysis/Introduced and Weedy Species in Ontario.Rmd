...
The botanical informatics group has been hard at work mobilizing data buried inside the Flora of North America’s volumes. We’ve created a Semantic MediaWiki instance to house the data we’ve extracted. 

Querying the Flora is *now easier than ever*. You can run queries with the Semantic MediaWiki Semantic search interface or programmatically with your data science language of choice (R or Python).

Here, I will investigate the number of introduced and weedy species in my home province, Ontario. I want to know:
...
This exploration leverages **Flora of North America data**. I’ll do it in R.
...
First, I'll browse the sample queries available on the Flora of North America Semantic MediaWiki platform. To get the data I want, I’ll simply modify the sample queries.

I want to find the list of taxa that are found in Ontario (Distribution = Ont.). Additionally, I want to know if a plant is described as weedy or introduced (Special status = Introduced, Special status = Weedy)*. I’ll download taxonomic information (Taxon family, Taxon rank, Taxon parent: liiinksss) to explore the taxonomic breakdown of my results.

Putting this all together using the Semantic search query syntax required to query Semantic MediaWiki, I get:
...
ont_spp <- read_csv("ontario_species.csv")
```

# Clean the data

The introduction status of a taxon provided by the Flora of North America is NA if no information has been provided by the treatment author for a taxon. I am going to make the assumption that if no information has been provided, the plant is **not introduced**.

Let's rename this value to something more readable.

```{r datacleaning}
ont_spp <- select(ont_spp, -X1) # Remove imported row indices
names(ont_spp) <- names(ont_spp) %>% tolower %>% sub(" ", "_", .) # clean up colnames
...
Due to limitations in data capture, [special status (i.e., weedy, endemic status) is only available in in the following volumes: V3, V6, V9, V12, V17, V28](http://dev.floranorthamerica.org/Sample_Queries#Special_Status).
...
As a result, the conclusions of my work will be biased.
...
Within the ~`r round((ont_spp %>% filter(!is.na(special_status)) %>% distinct %>% nrow/ont_spp %>% distinct %>% nrow) * 100, 0)`% of taxa with special status data, we see the breakdown of status below.
...
counts_to_plot <- inner_join(family_introduced_counts, family_native_counts, by = "taxon_family")
names(counts_to_plot) <- c("taxon_family", "num_introduced", "num_native")
counts_to_plot <- melt(counts_to_plot) %>% group_by(variable) %>% dplyr::arrange(desc(value), .by_group = TRUE)

ggplot(data = counts_to_plot, aes(x = reorder(taxon_family, value), y = value, fill = variable)) + 
...
counts_to_plot <- inner_join(family_weedy_counts, family_non_weedy_counts, by = "taxon_family")
names(counts_to_plot) <- c("taxon_family", "num_weedy", "num_non_weedy")
counts_to_plot <- melt(counts_to_plot) %>% group_by(variable) %>% dplyr::arrange(desc(value), .by_group = TRUE)
...
The **Euphorbiaceae** family contains the most number of weedy species, in absolute terms and as a percentage of the totaly number of taxa in the group (total number: `r taxonomy_subset_special_status %>% filter(special_status == "Weedy" & taxon_family == "Euphorbiaceae") %>% nrow`, percentage of group: `r round(taxonomy_subset_special_status %>% filter(special_status == "Weedy" & taxon_family == "Euphorbiaceae") %>% nrow / (taxonomy_subset_special_status %>% filter(special_status != "Weedy" & taxon_family == "Euphorbiaceae") %>% nrow + taxonomy_subset_special_status %>% filter(special_status == "Weedy" & taxon_family == "Euphorbiaceae") %>% nrow) * 100, 2)`).
