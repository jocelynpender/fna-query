---
title: "Introduced Species in Ontario"
output:
  html_document:
    df_print: paged
---

This is an abridged version of the blog post found here.
...
The introduction status of a taxon provided by the Flora of North America is NA if no information has been provided by the treatment author. I am going to make the assumption that if no information has been provided, the plant is **not introduced**.
...
# Transform the data into long-format for ease of visualization
...
From the above plot, I see that 15% of taxa found in Ontario described in the Flora of North America are introduced species.

## How many plants in Ontario are weedy?

Due to limitations in data capture, [special status data are only available in in the following volumes: V3, V6, V9, V12, V17, V28](http://dev.floranorthamerica.org/Sample_Queries#Special_Status).
...
Most of my data comes from volumes 23 and 24, however special status data is not available for these volumes. You can see the volumes in blue above that have special status data.
...
Unfortunately, the conclusions of my work will be biased.
...
From the above, I see that **being weedy greatly increases the probability that a plant is an introduced species** (`r cond_prob`% vs. `r reg_prob`%). This makes sense given my understanding of what makes introduced species successful. 

## What is the taxonomic make-up of these two groups (introduced and weedy)?

Which families are the largest offenders? 
